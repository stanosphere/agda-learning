...
bool-monoid-∨ = record 
  ...
  where
    bool-assoc-lemma : (a b c : Bool) -> (a ∨ b) ∨ c ≡ a ∨ (b ∨ c)
    bool-assoc-lemma false b c = refl
    bool-assoc-lemma true b c = refl 
...
module Properties (M : Monoid) where
    ...
    expo-law-zero (suc x) = begin
      ε           ≡⟨ idR-reverse ε ⟩
      ε ⊕ ε       ≡⟨ cong (λ u -> ε ⊕ u) (expo-law-zero x) ⟩
      ε ⊕ (ε ^ x) ≡⟨ refl ⟩ -- a ^ (suc n) = a ⊕ (a ^ n) is the definition remmeber
      ε ^ (suc x) ∎
    ...
    expo-law-one (suc x) y n = begin
      (n ⊕ n ^ (x + y))     ≡⟨ cong (λ u -> n ⊕ u) (expo-law-one x y n) ⟩
      (n ⊕ (n ^ x ⊕ n ^ y)) ≡⟨ sym (assoc (n) (n ^ x) (n ^ y)) ⟩
      (n ⊕ n ^ x ⊕ n ^ y)   ∎
    ...
    expo-law-two x 0 n = begin
      n ^ (x * 0) ≡⟨ cong (λ (u : ℕ) -> n ^ u) ( mult-zero-law x) ⟩
      n ^ 0       ≡⟨ refl ⟩
      ε           ≡⟨ expo-law-zero x ⟩
      ε ^ x       ∎ 
    expo-law-two (suc x) y n = begin
      n ^ (suc x * y)       ≡⟨ refl ⟩
      n ^ ((1 + x) * y)     ≡⟨ refl ⟩
      n ^ (y + x * y)       ≡⟨ expo-law-one y (x * y) n ⟩
      n ^ y ⊕ n ^ (x * y)   ≡⟨ cong (λ u -> n ^ y ⊕ u) (expo-law-two x y n) ⟩
      (n ^ y) ⊕ (n ^ y) ^ x ≡⟨ refl ⟩ 
      (n ^ y) ^ (1 + x)     ≡⟨ refl ⟩
      (n ^ y) ^ suc x       ∎ 
